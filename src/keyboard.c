...
/* minimalna mapa skan->ASCII (US), bez AltGr/caps; Backspace/Enter w tablicy */
...
void keyboard_init(void){
    /* włącz interface klawiatury i skanowanie – nie każdy BIOS robi to sam */
...
/* obsługa Shift: 0x2A/0x36 make, 0xAA/0xB6 break */
...
void keyboard_handler(void){
    ...
    if (kbd_can_read()){
        ...
        if (sc == 0xE0) { /* extended prefix – pomijamy w tym minimalu */ }
        else if (sc == 0x2A || sc == 0x36) { shift = 1; }       /* LSHIFT / RSHIFT make */
        else if (sc == 0xAA || sc == 0xB6) { shift = 0; }       /* LSHIFT / RSHIFT break */
        else if (!(sc & 0x80)) {                                /* tylko make code */
            char c = (shift ? map_shift : map_norm)[sc];
            if (c) kb_enqueue(c);
        }
    }
    pic_send_eoi(1);
